...
function buildCaloriesHistory(meals: MealResponse[], days: number): DayCalories[] {
  ...
  // Grouper par semaine ou mois si grande période
  if (days <= 30) {
    return Object.entries(byDay)
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([date, kcal]) => ({
        date,
        date_display: new Date(date).toLocaleDateString("fr-FR", { day: "2-digit", month: "2-digit" }),
  ...
  return Object.entries(byMonth)
    ...
    .map(([key, v]) => {
      ...
      return {
        date: key,
        date_display: new Date(Number(y), Number(m) - 1).toLocaleDateString("fr-FR", { month: "short" }),
        kcal: Math.round(v.total / v.count),
      };
    });
}
...
export default function StatsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`http://localhost:8003/users/${userId}/metrics`).then((r) => r.json()),
    ])
  ...
  async function handleAddMetric(e: React.FormEvent) {
    ...
    setFormError("");
    setFormSuccess("");
    if (!formPoids && !formSommeil && !formBpm) {
      setFormError("Renseignez au moins une valeur.");
      return;
    }
    setFormLoading(true);
    try {
      ...
      setMetrics((prev) => {
        const filtered = prev.filter(
          (m) => m.date_mesure?.slice(0, 10) !== updated.date_mesure?.slice(0, 10)
        );
        return [...filtered, updated].sort((a, b) =>
          (a.date_mesure || "").localeCompare(b.date_mesure || "")
        );
      });
      setFormSuccess("Données enregistrées !");
      setFormPoids("");
      setFormSommeil("");
      setFormBpm("");
  ...
  const caloriesHistory = useMemo(() => buildCaloriesHistory(meals, calDays), [meals, calDays]);
      ...
      <header className="px-5 pt-6 pb-4">
        <div className="flex justify-between items-start max-w-md mx-auto">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Votre évolution</p>
          </div>
      ...
      <section className="px-5 pb-24 space-y-5 animate-fade-in max-w-md mx-auto">
        ...
        <div className="grid grid-cols-3 gap-3">
          <div className="p-3 bg-card rounded-2xl border border-border text-center space-y-1">
            ...
            <p className="text-[10px] text-muted-foreground">Moy. kcal/j</p>
          </div>
          <div className="p-3 bg-card rounded-2xl border border-border text-center space-y-1">
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <span className="font-semibold text-sm">Saisir ma progression</span>
          </div>
          {showForm
        ...
        {showForm && (
          <form onSubmit={handleAddMetric} className="p-4 bg-card border border-primary/20 rounded-2xl space-y-4 animate-scale-in">
            <h3 className="font-semibold text-sm">Nouvelle entrée</h3>
            <input
              type="date"
              value={formDate}
            ...
            <div className="grid grid-cols-3 gap-2">
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-muted-foreground">Sommeil (h)</label>
                <input
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-muted-foreground">BPM repos</label>
                <input
        ...
        <div className="p-5 bg-card border border-border rounded-2xl space-y-4">
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={caloriesHistory}>
                  ...
                  <Tooltip
                    ...
                    formatter={(v: number) => [`${v} kcal`, calDays > 30 ? "Moy. kcal/j" : "Calories"]}
                  />
          ...
          {calDays > 30 && (
            <p className="text-[10px] text-muted-foreground text-center">
              Valeurs moyennes par {calDays <= 180 ? "semaine" : "mois"}
            </p>
          )}
        </div>
        ...
        {/* Répartition repas par type */}
        {!loading && meals.length > 0 && (
          <div className="p-5 bg-card border border-border rounded-2xl space-y-4">
            <h3 className="font-semibold">Repas enregistrés</h3>
            <div className="grid grid-cols-2 gap-3">
              {(["petit_dejeuner", "dejeuner", "diner", "collation"] as const).map((type) => {
                const label = { petit_dejeuner: "Petit-dej", dejeuner: "Déjeuner", diner: "Dîner", collation: "Collation" }[type];
                ...
                return (
                  <div key={type} className="p-3 bg-background rounded-xl border border-border space-y-1">
                    ...
                    <p className="text-[10px] text-muted-foreground">{pct}% du total</p>
                  </div>
                );
              })}
            </div>
